...
def test_bert_dataset_transform():
    ...
    bert_cls_dataset_t = t.BERTDatasetTransform(tokenizer, [label_cls], 15, pad=True,
    ...
    label_reg = 0.2
    bert_reg_dataset_t = t.BERTDatasetTransform(tokenizer, [label_reg], 15, pad=True,
                                                pair=True, label_dtype='float32')
    token_ids, length, type_ids, label_value = bert_cls_dataset_t((text_a, text_b, label_cls))
    assert all(token_ids == valid_token_ids)
    assert length == len(vocab_tokens) + 3
    assert all(type_ids == valid_type_ids)
    assert all(label_ids == np.array([label_cls], dtype=np.int32))
