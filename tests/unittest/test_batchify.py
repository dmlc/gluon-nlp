            ...
            for length_min, length_max in [(3, 4), (3, 7)]:
                for pad_val in [-1, 0]:
                    for dtype in [np.uint8, np.int32, np.int64, np.float16, np.float32, np.float64]:
                        ...
                        batch_data_use_mx, valid_length_use_mx = batchify_fn([mx.nd.array(ele, dtype=dtype) for ele in random_data_npy])
                        ...
                        for pad_index in [[0], [1], [2], [0, 1], [1, 2], [0, 1, 2]]:
                            ...
                            ret_use_mx = batchify_fn([tuple(mx.nd.array(ele[i], dtype=dtype) for i in range(TOTAL_ELE_NUM)) for ele in random_data_npy])
                            ...
                            for i in range(batch_size):
                                for j in range(TOTAL_ELE_NUM):
                                    ...
                                    _verify_padded_arr(batch_data[i], random_data_npy[i][j], axis, pad_val, pad_length, dtype)
