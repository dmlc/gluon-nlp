...
from gluonnlp.data import sampler
...
def test_sorted_sampler():
    ...
    sample_ret = list(sampler.SortedSampler([ele.shape[0] for ele in dataset]))
...
@pytest.mark.parametrize('bucket_scheme', [sampler.ConstWidthBucket(),
                                           sampler.LinearWidthBucket(),
                                           sampler.ExpWidthBucket()])
...
def test_fixed_bucket_sampler(seq_lengths, ratio, shuffle, num_buckets, bucket_scheme,
    ...
    sampler = sampler.FixedBucketSampler(seq_lengths,
                                         batch_size=8,
...
def test_fixed_bucket_sampler_with_single_key(bucket_keys):
    seq_lengths = [np.random.randint(10, 100) for _ in range(N)]
    sampler = sampler.FixedBucketSampler(seq_lengths, batch_size=8, num_buckets=None,
...
def test_sorted_bucket_sampler(seq_lengths, mult, batch_size, shuffle):
    sampler = sampler.SortedBucketSampler(sort_keys=seq_lengths,
