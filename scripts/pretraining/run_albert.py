"""Pretraining Example for Albert Model. We pretrain the albert model on OpenWebText dataset"""
import os
import sys
import time
import shutil
...
from gluonnlp.models.albert import AlbertModel, get_pretrained_albert, AlbertForMLM
...
def parse_args():
    ...
    parser.add_argument('--output_dir', type=str, default='albert_owt',
...
def get_pretraining_model(model_name, ctx_l,
    ...
    """
    cfg, tokenizer, backbone_path, mlm_params_path = get_pretrained_albert(
        model_name, load_backbone=False)
    cfg = AlbertModel.get_cfg().clone_merge(cfg)
    cfg.defrost()
    ...
    model = AlbertForMLM(cfg, weight_initializer=TruncNorm(stdev=0.02), prefix='Pretrain_')
    model.load_parameters(mlm_params_path, ctx=ctx_l)
        ...
        while not is_last_batch:
            ...
            for sample, ctx in zip(sample_l, ctx_l):
                ...
                with mx.autograd.record():
                    contextual_embeddings, pooled_out, mlm_scores = model(
                        masked_input_ids, segment_ids, valid_lengths, masked_positions)
            ...
            if (batch_id + 1) % args.num_accumulated == 0 or is_last_batch:
                ...
                if step_num % log_interval == 0 and local_rank == 0:
                    ...
                    if args.do_eval:
                        evaluation(writer, step_num, masked_input, mlm_scores)
...
def evaluation(writer, step_num, masked_input, mlm_scores):
    length_masks = masked_input.masks
    unmasked_tokens = masked_input.unmasked_tokens
    masked_weights = masked_input.masked_weights
