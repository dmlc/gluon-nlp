...
def get_embedding(counter, data_type, limit=-1, emb_file=None, size=None, vec_size=None):
    ...
    token2idx_dict[NULL] = opt.PAD
    token2idx_dict[OOV] = opt.UNK
...
def build_features(examples, data_type, word2idx_dict, char2idx_dict):
    ...
    """
    if data_type == 'train':
        para_limit = opt.max_context_sentence_len
        ques_limit = opt.max_question_sentence_len
    else:
        para_limit = 1000
        ques_limit = 100
    ans_limit = opt.max_answer_len
    char_limit = opt.max_character_per_word
...
def save(filename, obj, message=None):
    ...
    if message is not None:
        ...
        with open(opt.data_path+filename, 'w') as fh:
            json.dump(obj, fh)
...
def prepro():
    ...
    word_emb_file = opt.glove_file_name
    char_emb_dim = opt.char_emb_dim
    word_emb_dim = opt.word_emb_dim
    train_examples = process_file(
        opt.data_path+'train-v1.1.json', 'train', word_counter, char_counter)

    word_emb_mat, word2idx_dict = get_embedding(
        word_counter,
        'word',
        emb_file=opt.data_path+word_emb_file,
        size=2.2e6,
        vec_size=word_emb_dim
    )
    opt.word_corpus = len(word2idx_dict)
    char_emb_mat, char2idx_dict = get_embedding(
        char_counter, 'char', emb_file=None, size=None, vec_size=char_emb_dim)
    opt.char_corpus = len(char2idx_dict)
    dev_examples = process_file(
        opt.data_path+'dev-v1.1.json', 'dev', word_counter, char_counter)
