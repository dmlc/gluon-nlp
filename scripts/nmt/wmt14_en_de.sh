...
for f in ${OUTPUT_DIR}/*.de; do
  echo "Tokenizing $f..."
  cat $f | ${OUTPUT_DIR}/mosesdecoder/scripts/tokenizer/normalize-punctuation.perl -l de | \
    ${OUTPUT_DIR}/mosesdecoder/scripts/tokenizer/remove-non-printing-char.perl | \
    ${OUTPUT_DIR}/mosesdecoder/scripts/tokenizer/tokenizer.perl -q -no-escape -protected ${OUTPUT_DIR}/mosesdecoder/scripts/tokenizer/basic-protected-patterns \
      -l de -threads 8 > ${f%.*}.tok.de &
done

for f in ${OUTPUT_DIR}/*.en; do
  echo "Tokenizing $f..."
  cat $f | ${OUTPUT_DIR}/mosesdecoder/scripts/tokenizer/normalize-punctuation.perl -l en | \
    ${OUTPUT_DIR}/mosesdecoder/scripts/tokenizer/remove-non-printing-char.perl | \
    ${OUTPUT_DIR}/mosesdecoder/scripts/tokenizer/tokenizer.perl -q -no-escape -protected ${OUTPUT_DIR}/mosesdecoder/scripts/tokenizer/basic-protected-patterns \
      -l en -threads 8 > ${f%.*}.tok.en &
done
