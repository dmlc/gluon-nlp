...
parser.add_argument(
    '--model_mode', choices=['rand', 'static', 'non-static', 'multichannel'],
...
if args.data_name in ('MR', 'Subj', 'CR', 'MPQA'):
    vocab, max_len, output_size, train_dataset, train_data_lengths = process_data.load_dataset(
        args.data_name)
elif args.data_name == 'TREC':
    vocab, max_len, output_size, train_dataset, train_data_lengths, test_dataset, \
test_data_lengths = process_data.load_dataset(args.data_name)
else:
    vocab, max_len, output_size, train_dataset, train_data_lengths, test_dataset,\
...
def check_metrics(metrics):
    ...
    if isinstance(metrics, mx.metric.CompositeEvalMetric):
        metrics = [m for metric in metrics.metrics for m in check_metrics(metric)]
    elif isinstance(metrics, mx.metric.EvalMetric):
        metrics = [metrics]
    else:
        metrics = metrics or []
        if not all([isinstance(metric, mx.metric.EvalMetric) for metric in metrics]):
...
class GetValMetricHandler(EpochEnd):
    ...
    def epoch_end(self, estimator, *args, **kwargs):  # pylint: disable=W0621
        for metric in self.metrics:
            metric_name, metric_val = metric.get()
            if 'validation' in metric_name:
                self.metric_history.setdefault(metric_name, []).append(metric_val)
...
class SentimentAnalysisCNNBatchProcessor(BatchProcessor):
    ...
    def evaluate_batch(self, estimator, val_batch, batch_axis=0):
        ...
        pred = [estimator.val_net(data) for data in data_list]
        loss = [estimator.val_loss(y_hat, y) for y_hat, y in zip(pred, label_list)]
    ...
    def fit_batch(self, estimator, train_batch, batch_axis=0):
        ...
        with autograd.record():
            ...
            loss = [estimator.loss(y_hat, y).mean() for y_hat, y in zip(pred, label_list)]
        for l in loss:
            l.backward()
        pre_labels = [nd.argmax(pred_pro, axis=1) for pred_pro in pred]
        return data_list, label_list, pre_labels, loss
...
def k_fold_cross_valid(k, all_dataset):
    ...
    for i in range(k):
        ...
        _train_dataset = all_dataset[:i * fold_size] + all_dataset[(i + 1) * fold_size:]
        _train_dataloader = DataLoader(dataset=_train_dataset, batch_size=args.batch_size,
                                       shuffle=True)
        _val_dataloader = DataLoader(dataset=_val_dataset, batch_size=args.batch_size,
                                     shuffle=False)
        _est = Estimator(net=net, loss=loss_fn, train_metrics=[mx.metric.Loss()], trainer=trainer,
                         context=context, val_loss=loss_fn,
        ...
        _est.fit(train_data=_train_dataloader, val_data=_val_dataloader, epochs=args.epochs,
                 event_handlers=[get_val_metric_handler])  # Add the event handlers
...
est = Estimator(net=net, loss=loss_fn, train_metrics=[mx.metric.Loss()], trainer=trainer,
                context=context, val_loss=loss_fn, val_metrics=[mx.metric.Accuracy()],
                batch_processor=SentimentAnalysisCNNBatchProcessor())
if __name__ == '__main__':
    if args.data_name == 'TREC':
        random.shuffle(train_dataset)
        sp = len(train_dataset) // 10
        train_dataloader = DataLoader(dataset=train_dataset[sp:], batch_size=args.batch_size,
                                      shuffle=True)
        val_dataloader = DataLoader(dataset=train_dataset[:sp], batch_size=args.batch_size,
                                    shuffle=False)
        est.fit(train_data=train_dataloader, val_data=val_dataloader,
                epochs=args.epochs)  # Add the event handlers
    elif args.data_name == 'SST-1' or args.data_name == 'SST-2':
        train_dataloader = DataLoader(dataset=train_dataset, batch_size=args.batch_size,
                                      shuffle=True)
        val_dataloader = DataLoader(dataset=dev_dataset, batch_size=args.batch_size, shuffle=False)
        est.fit(train_data=train_dataloader, val_data=val_dataloader,
                epochs=args.epochs)  # Add the event handlers
    else:
