...
parser.add_argument(
    ...
    'Default is None')
parser.add_argument('--gpu', type=int, default=None, help='Which gpu for finetuning.')
parser.add_argument(
    '--task_name', type=str, choices=tasks.keys(),
    help='The name of the task to fine-tune. Choices include MRPC, QQP, '
    'QNLI, RTE, STS-B, CoLA, MNLI, WNLI, SST.')
parser.add_argument(
    '--bert_model', type=str, default='bert_12_768_12',
    choices=['bert_12_768_12', 'bert_24_1024_16', 'roberta_12_768_12',
             'roberta_24_1024_16'], help='The name of pre-trained BERT model to fine-tune')
parser.add_argument(
    '--bert_dataset', type=str, default='book_corpus_wiki_en_uncased', choices=[
        'book_corpus_wiki_en_uncased', 'book_corpus_wiki_en_cased',
        'openwebtext_book_corpus_wiki_en_uncased', 'wiki_multilingual_uncased',
        'wiki_multilingual_cased', 'wiki_cn_cased', 'openwebtext_ccnews_stories_books_cased'
    ], help='The dataset BERT pre-trained with.')
parser.add_argument('--pretrained_bert_parameters', type=str, default=None,
...
if args.pretrained_bert_parameters:
    logging.info('loading bert params from %s', args.pretrained_bert_parameters)
    nlp.utils.load_parameters(model.bert, args.pretrained_bert_parameters, ctx=ctx,
                              ignore_extra=True, cast_dtype=True)
if args.model_parameters:
    logging.info('loading model params from %s', args.model_parameters)
    nlp.utils.load_parameters(model, args.model_parameters, ctx=ctx, cast_dtype=True)
...
def preprocess_data(tokenizer, task, batch_size, dev_batch_size, max_len, vocab, pad=False):
    ...
    trans = BERTDatasetTransform(tokenizer, max_len, vocab=vocab, class_labels=task.class_labels,
                                 label_alias=task.label_alias, pad=pad, pair=task.is_pair,
                                 has_label=True)
    ...
    test_trans = BERTDatasetTransform(tokenizer, max_len, vocab=vocab, class_labels=None, pad=pad,
                                      pair=task.is_pair, has_label=False)
...
train_data, dev_data_list, test_data_list, num_train_examples = preprocess_data(
    bert_tokenizer, task, args.batch_size, args.dev_batch_size, args.max_len, vocabulary, args.pad)
...
def train(metric):
    ...
    trainer = gluon.Trainer(all_model_params, 'bertadam', optimizer_params, update_on_kvstore=False)
    if args.dtype == 'float16':
    ...
    for epoch_id in range(args.epochs):
        ...
        if not args.only_inference:
            # save params
            ckpt_name = 'model_bert_{0}_{1}.params'.format(args.task_name, epoch_id)
