...
with open(gluon_vocab_path, 'w') as f:
    f.write(vocab.to_json())
    logging.info("vocab file saved to %s. hash = %s"%
                 (gluon_vocab_path, hash_full))
...
for name in params:
    try:
        ...
        if name not in mx_tensors:
            ...
            logging.info('%s cannot initialize %s. Expect shape = %s, but found %s %s'%
                  (bar, name, params[name].shape, arr.shape, bar))
