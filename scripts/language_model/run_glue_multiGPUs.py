...
model = XLNetClassifier(xlnet_base, ctx, dropout=0.1, num_classes=num_classes)

# initialize classifier
if not model_parameters:
    model.classifier.initialize(init = initializer, ctx=ctx)
    model.pooler.initialize(init = initializer, ctx = ctx)
...
def train(metric):
    ...
    if not only_inference:
        logging.info('Now we are doing XLNet classification training on %s!', ctx)
