...
SquadXLNetFeautre = collections.namedtuple('SquadXLNetFeautre', [
    ...
    'tok_start_to_orig_index', 'tok_end_to_orig_index', 'token_is_max_context',
    'input_ids', 'p_mask', 'segment_ids', 'start_position', 'end_position',
    'paragraph_text', 'paragraph_len', 'is_impossible'
])
...
def convert_examples_to_features(example,
    ...
    token_is_max_context = [{
        p: check_is_max_context(doc_spans_indices, i,
    ...
    cur_tok_start_to_orig_index = [[
        tok_start_to_orig_index[p + st] for p in range(len(doc_span))
    ] for doc_span, (st, ed) in zip(doc_spans, doc_spans_indices)]
    cur_tok_end_to_orig_index = [[
        tok_end_to_orig_index[p + st] for p in range(len(doc_span))
    ] for doc_span, (st, ed) in zip(doc_spans, doc_spans_indices)]
    ...
    features = [
        SquadXLNetFeautre(example_id=example.example_id,
                          ...
                          tok_start_to_orig_index=t2st,
                          tok_end_to_orig_index=t2ed,
...
def _apply_gradient_decay():
    ...
    """
    parameter_not_included = [
        'seg_emb', 'query_key_bias', 'query_emb_bias', 'query_seg_bias'
    ]
