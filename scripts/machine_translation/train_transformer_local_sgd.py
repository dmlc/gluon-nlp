...
from transformer_local_sgd import LocalSGDTrainer
...
parser.add_argument('--local_sgd_interval', type=int, default=0, help='the number of local iterations of local SGD (initial value)')
...
def train():
    ...
    # local sgd
    if args.local_sgd_epochs is not None:
        local_sgd_epochs = [int(i) for i in args.local_sgd_epochs.split(',')]
    else:
        local_sgd_epochs = None
    if args.local_sgd_schedule is not None:
        local_sgd_schedule = [int(i) for i in args.local_sgd_schedule.split(',')]
    else:
    ...
    trainer = LocalSGDTrainer(model.collect_params(), args.optimizer,
                            ...
                            local_sgd_interval=local_sgd_interval, local_sgd_regularization=args.local_sgd_regularization, 
    ...
    for epoch_id in range(args.start_epoch, args.epochs):
        ...
        if local_sgd_epochs is not None and epoch_id in local_sgd_epochs:
            new_local_sgd_interval = local_sgd_schedule[local_sgd_epochs.index(epoch_id)]
            if new_local_sgd_interval <= 1:
                new_local_sgd_interval = 1
                if args.start_epoch != epoch_id:
                    if not trainer._is_states_initialized:
                        trainer.init_states()
                    trainer.allreduce_states()
            trainer._local_sgd_interval = new_local_sgd_interval
            local_sgd_interval = new_local_sgd_interval

        for batch_id, seqs \
                in enumerate(train_data_loader):

            if (local_sgd_interval > 1 or local_sgd_epochs is not None) and epoch_id == 0 and batch_id == grad_interval:
            ...
            if batch_id % grad_interval == grad_interval - 1 or\
                ...
                step_size = loss_denom / args.batch_size / 100.0
