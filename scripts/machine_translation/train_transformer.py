            ...
            if accum_count == args.num_accumulated or is_last_batch:
                ...
                if local_rank == 0 and \
                   (n_epoch_train_iters % args.log_interval == 0 or is_last_batch):
                    ...
                    writer.add_scalar('train_loss', log_avg_loss, n_train_iters)
                    writer.add_scalar('lr', trainer.learning_rate, n_train_iters)
                    log_start_time = time.time()
                    log_avg_loss = 0
                    log_loss_denom = 0
                    log_wc = 0
                if local_rank == 0 and \
                   (args.max_update > 0 and n_train_iters % args.save_interval_update == 0):
                    ...
                    writer.add_scalar('valid_loss', avg_valid_loss, n_train_iters)
