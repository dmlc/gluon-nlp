...
parser.add_argument('--local_sgd', type=int, default=0, help='the number of local iterations of local SGD')
...
trainer = gluon.Trainer(model.collect_params(), args.optimizer,
                            {'learning_rate': args.lr, 'beta2': 0.98, 'epsilon': 1e-9}, local_sgd=args.local_sgd, local_sgd_regularization=args.local_sgd_regularization, local_sgd_regularization_interval=args.local_sgd_regularization_interval)

rescale_loss = 100
parallel_model = ParallelTransformer(model, label_smoothing, loss_function, rescale_loss)
# parallel_model = ParallelTransformer(model, label_smoothing, loss_function, rescale_loss, amp=amp, trainer=trainer)
...
def train():
    ...
    for epoch_id in range(args.epochs):
        ...
        for batch_id, seqs \
            ...
            # if epoch_id == 0 and batch_id == 100:
            #     # amp
            #     mx.nd.waitall()
            #     logging.info('[Epoch {} Batch {}/{}] Activate amp'.format(epoch_id, batch_id + 1))
